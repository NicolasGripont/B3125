...
public class ServerMultiThread extends Thread {
    ...
    /**
     * Méthode exécutée par le thread. Boucle tant que server.isRunning() est true. Attend la connexion d'utilisateur, créer un ServerThread à la connexion d'un utilisateur et lance ce ServerThread.
     */
    public void run(){        
        try {
    ...
    /**
     * Méthode d'intéruption du thread. Ferme la socket server et appelle la méthode interrupt() de chaque ServerThreads (d'écoute côté serveur de chaque utilisateurs).
     */
    @Override
