...
public class Client implements ClientInterface{
     ...
     * On crée un stub pour le message écrit par le client et on l'envoie au serveur. 
     ...
     * Le client établit une connection au RMI registry crée dans la classe Server.java. Si la tentative de connexion réussi, le client est rajouté à la liste de Client du serveur.
    ...
    public Boolean connect(String sAdress, String sPort, String name) {
        ...
        try {
            ...
            try
            {
                clientChatView.displayMessage(new Message(new Date(), "Error", "Impossible to connect."));
            } catch (Exception e2) {
                System.err.println("Client exception: " + e2.toString());
            }
            server = null;
            result = false;
        }
     ...
     * Le client se déconnecte du serveur. On supprime le lien vers le serveur et on supprime le client de la liste Clients dans la classe Server.java .
     */
     ...
     * On vérifie que le client est bien connecté au serveur (l'attribut "server" qui constitue la réference vers le serveur est non nul).
