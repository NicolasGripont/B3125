...
static int Save(const string & filePath, const unordered_map<string, Shape *> &shapes);
...
// Manual : Methods that creates a Segment.
// Return a pointer on Shape* if succeed
// Return nullptr if failed or if the shape is not valid
//
// Contract : None.
//

static Shape* CreateRectangle(const deque<string> & params);
// Manual : Methods that creates a Rectangle.
// Return a pointer on Shape* if succeed
// Return nullptr if failed or if the shape is not valid
//
// Contract : None.
...
// Manual : Methods that creates a ConvexPolygon.
// Return a pointer on Shape* if succeed
// Return nullptr if failed or if the shape is not valid
//
// Contract : None.
...
// Manual : Methods that creates a ComplexShape (Reunion or Intersection).
// Return a pointer on Shape* if succeed
// Return nullptr if failed or if the shape is not valid
//
// Contract : None.
...
// Manual : Split a line from file.
// Return a deque containing all tokens of line
//
// Contract : None
//
