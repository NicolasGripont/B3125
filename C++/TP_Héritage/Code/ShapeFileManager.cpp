...
//
{
    ...
    if(file)
    {
        try
        {
            ...
            getline(file,nbShapesStr);
            nbShapes = stoi(nbShapesStr); // try catch for this

            for(int i = 0; i < nbShapes && !file.eof(); i++)
            {
                getline(file,line);
...
int ShapeFileManager::Save(const string & filePath, const map<string,Shape*> & shapes)
// Algorithm :
//
{
    bool result = 0;
